...
def text2embedd2query(text_file, query):
    ...
    annoy_index = load_annoy_index_from_db(annoy_index_key)
    if annoy_index is None:
        ...
        save_annoy_index_to_db(annoy_index, annoy_index_key)
...
def save_annoy_index_to_db(annoy_index, key):
    serialized_index = pickle.dumps(annoy_index)
    redis_client.set(key, serialized_index)


def load_annoy_index_from_db(key):
    serialized_index = redis_client.get(key)
